'''Derived byteplay that can be loaded for different Python versions
'''

# This work is derived from byteplay.py, as developed by Noam Raphael, and
# released with the following copyright information...
#
...
try:
    ...
    raise RuntimeError('xbyteplay not properly loaded; need certain globals set')

if python_version not in ('2.4', '2.5', '2.6', '2.7'):
    raise RuntimeError("byteplay doesn't support Python version "+python_version)
...
#
# Copied from Python2.6 dis.py to avoid importing opcode specific
